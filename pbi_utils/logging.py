...
class Logging(object):
    ...
    def __init__(self, disable_external_logging=True) -> None:
        ...
        if self.disable_external_logging:
            self.disable()
    ...
    def trace(self, msg) -> None:
        ...
        if not self.trace_logger:
            self.trace_logger = self.__create_new_logger(
                TRACE, format=f"{Fore.MAGENTA}[%(levelname)s]{Fore.RESET} %(message)s"
            )
    ...
    def debug(self, msg) -> None:
        ...
        if not self.debug_logger:
            self.debug_logger = self.__create_new_logger(
                DEBUG, format=f"{Fore.CYAN}[%(levelname)s]{Fore.RESET} %(message)s"
            )
    ...
    def info(self, msg) -> None:
        ...
        if not self.info_logger:
            self.info_logger = self.__create_new_logger(
                INFO, format=f"{Fore.GREEN}[%(levelname)s]{Fore.RESET} %(message)s"
            )
    ...
    def warning(self, msg) -> None:
        ...
        if not self.warning_logger:
            self.warning_logger = self.__create_new_logger(
                WARNING, format=f"{Fore.YELLOW}[%(levelname)s]{Fore.RESET} %(message)s"
            )
    ...
    def error(self, msg) -> None:
        ...
        if not self.error_logger:
            self.error_logger = self.__create_new_logger(
                ERROR, format=f"{Fore.RED}[%(levelname)s]{Fore.RESET} %(message)s"
            )
