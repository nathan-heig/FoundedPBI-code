...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fine-tune a masked language model with LoRA"
    )
    parser.add_argument(
        "--dataset_base_dir",
        type=str,
        required=True,
        help="Base folder of the datasets",
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
        required=True,
        help="Dataset name (folder name inside `dataset_base_dir`)",
    )
    parser.add_argument(
        "--base_model_name",
        type=str,
        required=True,
        help="Base pretrained model name or path, which will be fine-tuned",
    )
    parser.add_argument(
        "--output_model_dir",
        type=str,
        required=True,
        help="Output folder to save the fine-tuned model",
    )
    parser.add_argument(
        "--output_model_name",
        type=str,
        required=True,
        help="Output model name (folder name inside `output_model_dir`)",
    )
    parser.add_argument(
        "--output_plots_dir",
        type=str,
        required=False,
        default="auto",
        help="Output folder to save training plots. If 'auto', plots will be saved inside `output_model_dir`/`output_model_name`/plots",
    )

    parser.add_argument(
        "--model_type",
        type=str,
        choices=["nt2", "dnabert2"],
        default="nt2",
        help="Type of model to fine-tune: 'nt2' for Nucleotide Transformer 2, 'dnabert2' for DNABERT2",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Number of processes for dataset tokenization",
    )
    parser.add_argument(
        "--max_length",
        type=int,
    ...
    parser.add_argument(
        "--per_device_batch_size", type=int, default=16, help="Batch size per GPU"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Disable bf16 training, use fp16 instead",
    )
    parser.add_argument(
    ...
    if num_gpus > 1:
        logger.error(
            "Multi-GPU finetuning will probably not train properly, continue at your own risk."
        )

    if cli_args.output_plots_dir == "auto":
        cli_args.output_plots_dir = os.path.join(
            cli_args.output_model_dir, cli_args.output_model_name, "plots"
        )

    if cli_args.model_type != "nt2":
        logger.warning(
            f"For now, only 'nt2' model type is fully supported. You selected '{cli_args.model_type}'. The execution will probably fail."
        )
...
def load_transformers_model_dnabert2(cli_args: argparse.Namespace):
    ...
    model = AutoModel.from_pretrained(
        cli_args.base_model_name, trust_remote_code=True, config=config
    )
...
def load_dataset(cli_args: argparse.Namespace, tokenizer, model_max_length: int):
    ...
    tokenized = dataset.map(
        tokenize, batched=True, remove_columns=["sequence"], num_proc=cli_args.num_proc
...
if __name__ == "__main__":
    ...
    trainer.save_model(
        os.path.join(cli_args.output_model_dir, cli_args.output_model_name)
    )

    plot_path = os.path.join(
        cli_args.output_plots_dir, f"training_loss_epoch{cli_args.epochs}.png"
    )
