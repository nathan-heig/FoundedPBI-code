...
class BranchMLP(nn.Module):
    ...
    """
    def __init__(self, 
                 mlp_params: list[int], initial_input_size: int, dropout: float = 0.2):
        """
...
class MLPClassifier(AbstractNNClassifier):
    ...
    """
    def __init__(self, bacterium_embed_dim: int, phage_embed_dim: int, bacterium_mlp_sizes: list[int] | str, phage_mlp_sizes: list[int] | str, dropout: float | str = 0.2, dense_dim: int | str = 128):
        
        super().__init__(bacterium_embed_dim, phage_embed_dim)

        # Sanity checks
        self._sanity_checks(bacterium_embed_dim, phage_embed_dim, bacterium_mlp_sizes, phage_mlp_sizes, dense_dim)
        ...
        # Branches
        self.bacteria_branch = BranchMLP(bacterium_mlp_sizes, bacterium_embed_dim, float(dropout))
        self.phage_branch = BranchMLP(phage_mlp_sizes, phage_embed_dim, float(dropout))

        # Compute flattened size
        bacterium_flat_size = bacterium_mlp_sizes[-1] if len(bacterium_mlp_sizes) > 0 else bacterium_embed_dim
        phage_flat_size = phage_mlp_sizes[-1] if len(phage_mlp_sizes) > 0 else phage_embed_dim
    ...
    def _sanity_checks(self, bacterium_embed_dim, phage_embed_dim, bacterium_mlp_sizes, phage_mlp_sizes, dense_dim):
        if isinstance(bacterium_mlp_sizes, list):
            for tpl in bacterium_mlp_sizes:
                assert isinstance(tpl, int), f"bacterium_conv_params must be a list of integers or a string. Got: {bacterium_mlp_sizes}"
        else:
            assert isinstance(bacterium_mlp_sizes, str), f"bacterium_conv_params must be either a list of integers or a string. Got: {bacterium_mlp_sizes}"
        if isinstance(phage_mlp_sizes, list):
            for tpl in phage_mlp_sizes:
                assert isinstance(tpl, int), f"phage_conv_params must be a list of integers or a string. Got: {phage_mlp_sizes}"
        else:
            assert isinstance(phage_mlp_sizes, str), f"phage_conv_params must be either a list of integers or a string. Got: {phage_mlp_sizes}"
...
class BasicMLPClassifier(MLPClassifier):
    def __init__(self, bacterium_embed_dim: int, phage_embed_dim: int, mlp_params: list[int] | str, dropout: float | str = 0.5):
        ...
        self.mlp = BranchMLP(self.params, bacterium_embed_dim + phage_embed_dim, float(dropout))
