...
class BranchCNN(nn.Module):
    ...
    def __init__(self, conv_params: list[tuple[int, int, int]]):
        """
...
class CNNClassifier(AbstractNNClassifier):
    ...
    def __init__(
        ...
        phage_embed_dim: int,
        bacterium_conv_params: list[tuple[int, int, int]] | str,
        phage_conv_params: list[tuple[int, int, int]] | str,
        dense_dim: int | str = 128,
        dense_dropout: float | str = 0.5,
    ):
        ...
        self.bacteria_feat_dim = self._get_flatten_size(
            bacterium_embed_dim, self.bacteria_branch
        )
        ...
        if isinstance(bacterium_conv_params, list):
            for tpl in bacterium_conv_params:
                assert (
                    ...
                    and len(tpl) == 3
                    and all(isinstance(x, int) for x in tpl)
                ), f"bacterium_conv_params must be a list of tuples of three integers or a string. Got: {bacterium_conv_params}"
        else:
            assert isinstance(
                bacterium_conv_params, str
            ), f"bacterium_conv_params must be either a list of tuples of three integers or a string. Got: {bacterium_conv_params}"
        if isinstance(phage_conv_params, list):
            for tpl in phage_conv_params:
                assert (
                    ...
                    and len(tpl) == 3
                    and all(isinstance(x, int) for x in tpl)
                ), f"phage_conv_params must be a list of tuples of three integers or a string. Got: {phage_conv_params}"
        else:
            assert isinstance(
                phage_conv_params, str
            ), f"phage_conv_params must be either a list of tuples of three integers or a string. Got: {phage_conv_params}"
        assert isinstance(
            dense_dim, (int, str)
        ), f"dense_dim must be either an integer or a string. Got: {type(dense_dim)}: {dense_dim}"
        assert isinstance(
            dense_dropout, (float, str)
        ), f"dense_dropout must be either a float or a string. Got: {type(dense_dropout)}: {dense_dropout}"
        assert isinstance(
            bacterium_embed_dim, int
...
class BasicCNNClassifier(CNNClassifier):
    def __init__(
        ...
        bacterium_embed_dim: int,
        phage_embed_dim: int,
        cnn_params: list[tuple[int, int, int]] | str,
        dense_dim: int | str = 128,
        dense_dropout: float | str = 0.5,
    ):
        super().__init__(
            bacterium_embed_dim, phage_embed_dim, [], [], dense_dim, dense_dropout
        )
        ...
        self.flat_size = self._get_flatten_size(
            bacterium_embed_dim + phage_embed_dim, self.cnn
        )
