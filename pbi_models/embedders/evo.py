...
class EVO(AbstractModel):
        ...
        **kwargs,
        ...
        if self.load_model:
            self.tokenizer = AutoTokenizer.from_pretrained(
                f"togethercomputer/{model_name}", trust_remote_code=True, **kwargs
            )
            self.model = AutoModelForCausalLM.from_pretrained(
                f"togethercomputer/{model_name}", trust_remote_code=True
            )
    ...
    def _encode(self, dna_sequence: str) -> torch.Tensor:
        if len(dna_sequence) > self.max_seq_len:
            logger.debug(
                f"Found DNA sequence longer than max length ({len(dna_sequence)} vs {self.max_seq_len}), truncating it"
            )
            dna_sequence = dna_sequence[: self.max_seq_len]

        return self.tokenizer(dna_sequence, return_tensors="pt")["input_ids"].to(
            self.device
        )
